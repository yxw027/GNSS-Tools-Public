function [solName,posXyz, epochs, cov, ztd, vpl,velEnu] = parsePosSolFile(filename)
...
switch ext
    ...
    case '.txt' % novatel truth
        ...
        epochs = gps2epochs(novTruth(:,1),novTruth(:,2));
        llhNov    = novTruth(:,3:5);
        posXyz = llh2xyz(llhNov,1);
...
%         end
    ...
    case '.pos'% nrc ppp
        ...
        try
            ...
            ztdNrc0 = cellfun(@str2num,dataNRC.textdata(9:end,15));
        catch
            % new hotness
        ...
        dpos = -[0.8756
            -1.4496
            0.0382];
        posXyz = llh2xyz(llhNrc,1)+0*dpos';
