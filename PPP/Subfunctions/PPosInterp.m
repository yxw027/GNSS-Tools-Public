function [posP, velP,pPosInds,pPosPoly,ROut] = PPosInterp(PRNs, epochs,Ppos,Pprns,...
...
for idx = 1:length(PRNs)
    ...
    if multiConst || pPosInds(prn,1) ~= ind1 && pPosInds(prn,2) ~= ind2
        switch settings.orbitInterpMethod
            ...
            case 'lagrange'
                [posP(idx,1)] = LagrangeInter(Pepochsi(ind1:ind2)'-epochi,Pposi(ind1:ind2,1)',epochi-epochi+dti);
                [posP(idx,2)] = LagrangeInter(Pepochsi(ind1:ind2)'-epochi,Pposi(ind1:ind2,2)',epochi-epochi+dti);
                [posP(idx,3)] = LagrangeInter(Pepochsi(ind1:ind2)'-epochi,Pposi(ind1:ind2,3)',epochi-epochi+dti);
        end
        ...
        if velCalc
            if noVelData
                ...
                switch settings.orbitInterpMethod
                    ...
                    case 'lagrange'
                        posP1 = LagrangeInter(Pepochsi(ind1:ind2)',Pposi(ind1:ind2,1)',epochi+dt);
                        posP2 = LagrangeInter(Pepochsi(ind1:ind2)',Pposi(ind1:ind2,2)',epochi+dt);
                        posP3 = LagrangeInter(Pepochsi(ind1:ind2)',Pposi(ind1:ind2,3)',epochi+dt);
                end
        ...
        switch settings.orbitInterpMethod
            ...
            case 'lagrange'
                [posP(idx,1)] = LagrangeInter(Pepochsi(ind1:ind2)',Pposi(ind1:ind2,1)',epochi);
                [posP(idx,2)] = LagrangeInter(Pepochsi(ind1:ind2)',Pposi(ind1:ind2,2)',epochi);
                [posP(idx,3)] = LagrangeInter(Pepochsi(ind1:ind2)',Pposi(ind1:ind2,3)',epochi);
        end
        if velCalc
            if noVelData
                % No precise velocity data available... interpolate
...
%         end
    ...
    for pdx = 1:length(PRNs)
        ...
        if 0
            if constInds(pdx) == 2
                offset = [-545 0 2400]'*1e-3;
            else
                svni = prn2svn(PRNs(pdx),jd,constInds(pdx));
                %
                blocki = svn2block(svni,1,1);
