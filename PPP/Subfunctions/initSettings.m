...
%% Primary directory
% if contains(cd,'kgunning2')
%    % little lab computer
%    baseDir = 'E:\';
%    obsDir  = 'G:\';
%    tempDir = 'E:\Temp Folders\';
% elseif contains(cd,'kgunning')
%     baseDir = 'D:\';
%     baseDir2 = 'D:\';
% %     obsDir = 'D:\';
%     obsDir = 'O:\';
%     tempDir = 'D:\Temp Folders\';
% elseif contains(cd,'kazgu')
%     baseDir = ['C:\Users\kazgu\Desktop\GNSS Data\'];
%     tempDir = ['C:\Users\kazgu\Desktop\GNSS Data\Temp Dir\'];
%     % desktop plz
%     
%     baseDir = ['C:\Users\kazgu\Desktop\GNSS Data\'];
%     baseDir2 = ['C:\Users\kazgu\Desktop\GNSS Data\'];
%     obsDir = baseDir;
% elseif contains(cd,'Kaz')
%     baseDir = 'Z:\Data\';
%     baseDir2 = 'Z:\';
%     obsDir = 'O:\';
%     tempDir = 'D:\Temp Dir\';
% else
%     baseDir = [cd '\GNSS Data\'];
% end
...
% GPS----------------------------------------------------------------------
settings.preciseProdDir = fslash([baseDir 'Raw Data\Precise Products\']);

settings.waasNstbDir = fslash('D:\WAAS Data\');
settings.outputs = 'D:\GNSS Data\Outputs\Eph Comp and Hists\';
settings.outputs = fslash([baseDir 'Outputs\Eph Comp and Hists\']);
% RINEX GPS CNAV message files
settings.rnxCnavDir  = fslash([baseDir 'Raw Data\CNAV_BRDC\']);

% NGA APC Precise Ephemeris
settings.ngaApcDir   = fslash([baseDir 'Raw Data\Precise_eph\NGA_APC\']);
settings.igsFinDir   = fslash([baseDir 'Raw Data\Precise_eph\IGS_FINAL\']);

% High-rate precise clock
settings.gpsClkDir   = fslash([baseDir 'Raw Data\Precise_eph\']);
settings.gloClkDir   = fslash([baseDir 'Raw Data\Precise_eph\']);
settings.mgxClkDir   = fslash([baseDir 'Raw Data\Precise_eph\']);

% GLONASS------------------------------------------------------------------
% IGS Precise Ephemeris
settings.gloIgsDir  = fslash([baseDir 'Raw Data\Precise_eph\GLONASS\']);

% Receiver tropo solutions
settings.igsTropo = fslash([baseDir 'Raw Data\tropo\']);

% IAC Bulletins------------------------------------------------------------
settings.gloBulDir   = fslash([baseDir 'Raw Data\GLONASS_BULLETINS\']);

% GALILEO/BEIDOU-----------------------------------------------------------
% MGEX combined broadcast ephemerides
settings.rnxMgexNavDir = fslash([baseDir 'Raw Data\RINEX_NAV_MGEX\']);
...
settings.rnxMgexPephDir = fslash([baseDir 'Raw Data\Precise_eph\MGEX\']);

% Observation data
settings.rinexObsDir = fslash([obsDir 'RINEX_OBS\']);
settings.rinexObsMatDir = fslash([obsDir 'RINEX_OBS_MAT\']);

settings.mgxObsDir =fslash([obsDir 'RINEX_OBS_MGEX\']);
% settings.mgxObsDir =fslash(['E:\RINEX_OBS_MGEX\']);
% settings.mgxObsDir =fslash([baseDir '\Raw Data\RINEX_OBS_MGEX\']);
% settings.mgxObsMatDir = fslash([baseDir 'Raw Data\RINEX_OBS_MGEX_MAT\']);
% settings.mgxObsMatDir =fslash([baseDir 'GNSS Data\Obs\RINEX_OBS_MGEX_MAT\']);
settings.mgxObsMatDir =fslash([obsDir 'RINEX_OBS_MGEX_MAT\']);

% settings.mgxObsMatDir = fslash([baseDir 'Raw Data\RINEX_OBS_MGEX_MAT\']);
% settings.mgxObsMatDir =fslash(['E:\RINEX_OBS_MGEX_MAT\']);

settings.mgxHRObsDir = fslash([obsDir 'RINEX_HR_OBS_MGEX\']);
settings.mgxHRObsMatDir = fslash([baseDir 'GNSS Data\Obs\RINEX_HR_OBS_MGEX_MAT\']);
settings.mgxHRObsMatDir = fslash([obsDir 'RINEX_HR_OBS_MGEX_MAT\']);

%% Processed product directories
% SUGL---------------------------------------------------------------------
settings.suglGpsDir = fslash([baseDir 'Outputs\sugl\sugl_GPS\']);
settings.suglGloDir = fslash([baseDir 'Outputs\sugl\sugl_GLO\']);
settings.suglGalDir = fslash([baseDir 'Outputs\sugl\sugl_GAL\']);
settings.suglBdsDir = fslash([baseDir 'Outputs\sugl\sugl_BDS\']);

% Raw ephemeris comparisons------------------------------------------------
% GPS NGA-COD high rate clock interpolated comparisons
settings.ngaCodCmpDir = fslash([baseDir 'Outputs\Eph Comp\NGA_COD_CMP\']);
% GPS NGA low rate clock comparisons
settings.ngaCmpDir    = fslash([baseDir 'Outputs\Eph Comp\NGA_CMP\']);
% Galileo IGS low rate comparisons
settings.mgxGalCmpDir = fslash([baseDir 'Outputs\Eph Comp\MGX_GAL_CMP\']);
% Beidou IGS low rate comparisons
settings.mgxBdsCmpDir = fslash([baseDir 'Outputs\Eph Comp\MGX_BDS_CMP\']);


% Histograms---------------------------------------------------------------

% GPS statistics from comparisons
settings.histGpsDir = fslash([baseDir 'Outputs\histo\histo_GPS\']);
% Galileo statistics from comparisons
settings.histGalDir = fslash([baseDir 'Outputs\histo\histo_GAL\']);
% Beidou statistics from comparisons
settings.histBdsDir = fslash([baseDir 'Outputs\histo\histo_BDS\']);

% Receiver observation outputs---------------------------------------------
settings.rxObsDir   = fslash([baseDir 'Outputs\rxobs\']);

% GLONASS Ephemerides from UNAVCO logs
settings.uraGloDir     = fslash([baseDir 'Outputs\sugl\GLONASS Eph from T02\']);

% Daily reports
settings.ReportGpsDir = fslash([baseDir 'Outputs\Daily Reports\GPS Reports\']);
settings.ReportGloDir = fslash([baseDir 'Outputs\Daily Reports\GLO Reports\']);

%
settings.glonassUploadTimesDir = fslash([baseDir 'Outputs\misc\GLO Upload Times\']);

% Clock estimation outputs
settings.clockEstOut = fslash([baseDir 'Misc Outputs\Clock and DCB estimation\']);
