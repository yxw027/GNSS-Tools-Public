function [state,dstate,resids,measMat] = lsSolGnss(obs,corrData,PARAMS,settings,varargin)
...
[svPos,svVel] = corrData.propagate(measMat(:,1),measMat(:,2),obs.epochs(1)*ones(nMeas,1),settings);
...
for idx = 1:nIter
    ...
    if xNorm < closeThresh
        [svPos,svVel] = corrData.propagate(measMat(:,1),measMat(:,2),ttx,settings);
    end
    ...
    if xNorm < closeThresh
        ...
        usrLlh = xyz2llh(usrPos');
        
        % Update tropo delay
        trop0 = saastamoinen_model_SU(usrLlh(1), usrLlh(2), usrLlh(3), el*180/pi);
