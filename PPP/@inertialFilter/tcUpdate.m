function [extraInputs, measMatRemoved,measMatRemovedLow] = ...
    tcUpdate(obj,epoch,obs,corrData,PARAMS,settings,outStruc,extraInputs,measMask)
...
if tSinceInertialUpdate < dtKf
    ...
    if 1
        Q(obj.INDS_STATE.ATTITUDE,obj.INDS_STATE.ATTITUDE) = ...
            eye(3) * PARAMS.Q.gyro_noise_PSD.^2 * dtKf*0.01;
        
        Q(obj.INDS_STATE.VEL,obj.INDS_STATE.VEL)     =  ...
            eye(3) * PARAMS.Q.accel_noise_PSD.^2 * dtKf*0.01;
...
%         eye(3) * PARAMS.Q.VEL.^2 * dtKf;
    ...
    if strcmp(PARAMS.states.ionoMode,'L1DELAYSTATE')
    ...
    Q(obj.INDS_STATE.CLOCK_BIAS,obj.INDS_STATE.CLOCK_BIAS) = ...
        eye(length(obj.INDS_STATE.CLOCK_BIAS)).*(PARAMS.Q.RXB).^2 * dtKf;
    Q(obj.INDS_STATE.CLOCK_DRIFT,obj.INDS_STATE.CLOCK_DRIFT) = ...
        eye(length(obj.INDS_STATE.CLOCK_BIAS)).*(PARAMS.Q.DRXB).^2 * dtKf;
...
% cov_propagated = Phi_matrix * (cov + Q) *...
%     Phi_matrix' + Q;
...
if nMeas > 0
    ...
    if isempty(extraInputs)
        ...
        [svPos,svVel] = corrData.propagate(prnConstInds(:,1),prnConstInds(:,2),tRx,settings);
        ...
        [svPos,svVel] = corrData.propagate(prnConstInds(:,1),prnConstInds(:,2),epoch,settings);
        gRangeSv = sqrt(sum((obj.pos'-svPos).^2,2));
        
        tTx = epoch-gRangeSv./c;
        
        [svPos,svVel] = corrData.propagate(prnConstInds(:,1),prnConstInds(:,2),tTx,settings);
    ...
    if isempty(extraInputs)
        ...
        if ~isempty(extraInputs.measMatRemoved)
            indsRemove = [];
            for idx = 1:size(extraInputs.measMatRemoved,1)
                indsRemove = [indsRemove; find(measMat(:,1) == extraInputs.measMatRemoved(idx,1) & ...
                    measMat(:,2) == extraInputs.measMatRemoved(idx,2) & ...
                    measMat(:,3) == extraInputs.measMatRemoved(idx,3))];
            end
            
            % Remove from the measurement matrix and all of the new indexing
            % produced
            measMat(indsRemove,:)= [];
            nMeas = size(measMat,1);
            
            losInds(indsRemove) = [];
            indAmbStates(indsRemove)= [];
            indIonos(indsRemove) = [];
            indGloDcbs(indsRemove) = [];
        end
        
    end
    
    if ~isempty(measMask)
        measMask = permute(measMask,[1 3 2]);
        measMask = measMask(~isnan(measMask(:,1)),:);
        
        indsRemove = [];
        for idx = 1:size(measMask)
            indsRemove = [indsRemove; find(measMat(:,1) == measMask(idx,1) & measMat(:,2) == measMask(idx,2))];
        end
    ...
    for idx = 1:nMeas
        ...
        switch measTypei
            ...
            case 2
                ...
                % Carrier phase windup
                phWindi = phWind(losInd)*settings.c/freqi;
...
if nMeas > 0
    ...
    [H,delta_z,residsPost,K,measMat,~,measMatRemoved] = ...
