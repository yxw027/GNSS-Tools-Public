function measRemovedSlip = manageStatesMulti(obj,epoch,gnssMeas,PARAMS,outStruc)
...
for idx = 1:length(indsCpAvail)
    ...
    switch infoAdd(3)
        case 1
            ...
            if multiFilter
                for fdx = 2:nFilter
                    objList(fdx).state = [objList(fdx).state; ambEst0];
...
if PARAMS.states.iono && strcmp(PARAMS.states.ionoMode,'L1DELAYSTATE')
    ...
    for idx = 1:length(stateFound)
        ...
        if multiFilter
            for fdx = 2:nFilter
                objList(fdx).state = [objList(fdx).state; 0];
...
if PARAMS.states.RX_DCB_GPS
    ...
    if ~isfield(obj.INDS_STATE,'RX_DCB_GPS_INFO')
        ...
        if multiFilter
            for fdx = 2:nFilter
                objList(fdx).INDS_STATE.RX_DCB_GPS_INFO = [prnHighSnr sigRef];
            end
        end
    end
...
%     prnConstIndsSf = unique(measInfoAvail(measInfoAvail(:,4) < 100,[1 2]),'rows');
    ...
    for idx = 1:length(stateFound)
        ...
        covNew = zeros(indStateAdd);
        covNew(1:size(obj.cov,1),1:size(obj.cov,1)) = obj.cov;
        covNew(indStateAdd,indStateAdd) = PARAMS.SIGMA0.MP_CARR^2;
        obj.cov = covNew;
