...
while ~feof(fid)
    ...
    if contains(linetxt,'EPOCH OF FIRST MAP')
        ...
        startJdi = cal2jd(datai{1},datai{2},datai{3}+datai{4}/24+datai{5}/(24*60)+datai{6}/(24*60*60));
        [startDyi,startYri] = jd2doy(startJdi);
        startEpochi = jd2epochs(startJdi);
    end
    
    if contains(linetxt,'EPOCH OF LAST MAP')
        datai = textscan(linetxt,'%f %f %f %f %f %f');
        
        endJdi = cal2jd(datai{1},datai{2},datai{3}+datai{4}/24+datai{5}/(24*60)+datai{6}/(24*60*60));
        [endDyi,endYri] = jd2doy(endJdi);
        endEpochi = jd2epochs(endJdi);
    end
    ...
    if contains(linetxt,'START OF AUX DATA') && contains(upper(linetxt),'DIFFERENTIAL CODE BIASES')
        ...
        while ~contains(linetxt,'END OF AUX DATA')
            ...
            if contains(upper(linetxt),'PRN / BIAS / RMS')
                ...
                constIndi = convertConstIndName(consti,1);
                constiFull = convertConstIndName(constIndi);
                
                SVNs(ind) = prn2svn(prni,startJdi,constiFull);
                PRNs(ind) = prni;
                sites{ind} = NaN;
                domes{ind} = NaN;
                obs1{ind} = obs1i;
                ...
                satFlag(ind) = 1;
                const{ind} = consti;
                constInd(ind) = convertConstIndName(consti,1);
    ...
    if contains(linetxt,'START OF TEC MAP')
        ...
        epochs(mapi) = cal2epochs(datevecs(mapi,1),datevecs(mapi,2),...
