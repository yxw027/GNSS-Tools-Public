...
dayStartEpochs = jd2epochs(doy2jd(YearList,dayList));
...
for ddx = 1:length(YearList)
    ...
    Year = YearList(ddx);
    dayNum = dayList(ddx);
    [gpsWeek,gpsTow] = jd2gps(doy2jd(Year,dayNum));
    ...
    switch SOURCE_SELECT
        ...
        case 2
            ...
            filenamei = [settings.dcbMgexDir eval(ftpStruc.destFormat) eval(ftpStruc.fileFormat{1})];
            ...
            dcbDatai = [];
            if found && isempty(strFindCell(filesRead,filenamei))
                filesRead = [filesRead {filenamei}];
                if ~FLAG_NO_LOAD
                    dcbDatai = parseDcbBsxFile(filenamei);
        ...
        case 3
            ...
            filenamei = [settings.dcbMgexDir eval(ftpStruc.destFormat) eval(ftpStruc.fileFormat{2})];
            
            filenamei(strfind(filenamei,'*')) = [];
            
            dcbDatai = readIONEX(filenamei);
        case 4
