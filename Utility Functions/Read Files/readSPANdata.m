...
if 0
    ...
    if RAWIMUSXA && ~isempty(find(contains(textdata,'%RAWIMUSXA'), 1))       
        ...
        outData.RAWIMUSXA.epochs     = gps2epochs(outData.RAWIMUSXA.week,outData.RAWIMUSXA.tow);
...
%         outData.RAWIMUSXA.gyro = outData.gyro/100/100;  % this is probably wrong still.
    ...
    if RANGEA && ~isempty(find(contains(textdata,'#RANGEA'), 1))
        ...
        for idx = 1:nEpochs
           ...
           epochsi(idx) = gps2epochs(weeki,towi);
...
%                  = size(datai,1);
        ...
        epochsFull = gps2epochs(dataFull(:,2),dataFull(:,3));
