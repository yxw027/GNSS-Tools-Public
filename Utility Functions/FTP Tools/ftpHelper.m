function [dayChangei,YearChangei] = ftpHelper(inChoice,YearList,dayList,settings,varargin)
%% ftpHelper
...
switch inChoice
    case {1,2} % MGEX precise ephemeris
        ...
        if strcmp(center,'igs') 
            ...
            [YearChangei,dayChangei] = ftp_file_general(YearList,dayList,ftpStruc,center,exts{1});
...
%             [YearChangei,dayChangei] = ftp_file_general(YearList,dayList,ftpStruc,center,exts{1});
        ...
        elseif strcmp(center,'iac')
            ...
            jdChange = [];
            % try old format
            [YearChange1,dayChange1] = ftp_file_general(YearList,dayList,ftpStruc,center,exts{1});
            % try new format!
            [YearChange3,dayChange3] = ftp_file_general(YearList,dayList,ftpStruc3,center3,exts{2});
            
            changeOutput = unique(doy2jd([YearChange1; YearChange3],[dayChange1; dayChange3]));
            if ~isempty(changeOutput)
                [dayChangei,YearChangei] = jd2doy(changeOutput);
            end
        end
    ...
    case 3 % NGA APC Files
        ...
        [YearChangei,dayChangei] =ftp_file_general(YearList,dayList,ftpStruc);
        
    case 4 % IGS station position solutions
        % settings.igsStatPos = fslash([baseDir 'Raw Data\IGS_Stations_Final\']);
        ftpStruc.destDir      = settings.igsStatPos;
        ftpStruc.ftpSite      = 'cddis.gsfc.nasa.gov';
        ftpStruc.sourceFormat = '[''/pub/gps/products/'' num2str(gpsWeek) ''/'']';
        ftpStruc.destFormat   = '['''']';
%         ftpStruc.fileFormat   =  {'[''IGS'' num2str(mod(Year,100),''%02i'') ''P'' num2str(woy,''%02i'') ''_all.ssc.Z'']'};
    ...
    case 6 % IGS differential code bias data
        ftpStruc.destDir      = settings.dcbMgexDir;
        ftpStruc.ftpSite      = 'cddis.gsfc.nasa.gov';
    ...
    case 8 % MGEX Observation Files
        ...
        ftpStruc.ftpSite      = 'cddis.gsfc.nasa.gov';
        ftpStruc.sourceFormat = '[''/pub/gps/data/campaign/mgex/daily/rinex3/'' num2str(Year) ''/'' num2str(dayNum,''%03d'') ''/'' num2str(mod(Year,100),''%02i'') ''o/'' ]';
        ftpStruc.destFormat   = '[int2str(Year) ''\'' num2str(dayNum,''%03d'') ''/'']';
        
        if isempty(statCodes)
            ftpStruc.fileFormat   =  {'[''*o.Z'']'};
        else
            ftpStruc.fileFormat = strcat('[''',lower(statCodes),'*o.Z'']');
        end
        ftpStruc.unzipFlag    = 0;
        
        [YearChangei,dayChangei] =ftp_file_general(YearList,dayList,ftpStruc);
        
        % New RINEX 3 naming convention GSFC
        ...
        [YearChangei,dayChangei] =ftp_file_general(YearList,dayList,ftpStruc3);
        
        % Old RINEX naming convention GSFC
        ftpStruc.destDir      = settings.mgxObsDir;
        ftpStruc.ftpSite      = 'cddis.gsfc.nasa.gov';
    ...
    case 9 % GPS Broadcast Nav Message File
        ...
        %         ftpStruc.ftpSite      = 'garner.ucsd.edu';
        %         ftpStruc.sourceFormat = '[''/archive/garner/nav/'' num2str(Year) ''/'' num2str(dayNum,''%03d'') ''/'']';
        %         ftpStruc.destFormat   = '[int2str(Year) ''/'' num2str(dayNum,''%03d'') ''/'']';
        %         ftpStruc.fileFormat   =  {'[''*.Z'']'};
        %         ftpStruc.unzipFlag    = 0;
        %
        %         ftp_file_general(YearList,dayList,ftpStruc);
    ...
    case 10 % High rate MGEX observation files
        ...
        [YearChangei,dayChangei] =ftp_file_general_highrate(YearList,dayList,ftpStruc);
        ...
        [YearChangei,dayChangei] =ftp_file_general_highrate(YearList,dayList,ftpStruc3);
...
%             ftpStruc.fileFormat   =  {'[''brdm'' num2str(dayNum,''%03d'') ''0.'' num2str(mod(Year,100),''%02d'') ''p.Z'']'};
            ...
            [YearChangei,dayChangei] =ftp_file_general(YearList(inds1),dayList(inds1),ftpStruc);
        end
...
%             ftpStruc2017.fileFormat   =  {'[''brdm'' num2str(dayNum,''%03d'') ''0.'' num2str(mod(Year,100),''%02d'') ''p.Z'']'};
            ...
            [YearChangei,dayChangei] =ftp_file_general(YearList(inds0),dayList(inds0),ftpStruc2017);
        end
